...
Status: **Phase 0 accepted; Phases 1–3 complete and validated**
...
7. **Pinyin and stroke-count conflict precedence: Taiwan first.** Taiwan
   MOE-aligned CNS11643 readings are canonical. Phase 3 character stroke counts
...
| `cns11643-attributes-2026-08-05` | Canonical Taiwan Zhuyin, Pinyin, and stroke-sequence counts; later components, radicals, and stroke-order attributes | Approved, Taiwan OGDL 1.0. CNS character selection/forms follow Taiwan MOE standard-character tables. Three independently hashed Phase 1 members pass CRC; damaged unrelated members remain excluded. |
...
| Field group | Planned source(s) | Coverage assessment |
|---|---|---|
| Trad/simp and variants | Unihan plus CC-CEDICT | Strong; all one-to-many mappings require review notes. |
| Radical and residual strokes; total strokes | Unihan `kRSUnicode`; MOE 1996 `筆畫` canonical; CNS sequence length, Unihan `kTotalStrokes`, and Make Me a Hanzi comparison | Complete for the selected set. Never use the removed `kRSKangXi`; retain all count disagreements. |
...
| Pinyin, Jyutping, Korean/Japanese readings | Unihan and CC-CEDICT | Good but incomplete; preserve multiple and conflicting readings. |
| Zhuyin | CNS11643 where attested; otherwise CLDR transform from sourced Pinyin | Reproducible with dual provenance, but lexical exceptions remain gaps. |
| Definitions | Unihan and CC-CEDICT | Good for the frequency-selected set, not guaranteed complete. |
...
| Field group | Planned source(s) | Coverage assessment |
|---|---|---|
| Trad/simp, pinyin, definitions | CC-CEDICT | Viable only for matched entries. |
...
| Zhuyin | CLDR transform from sourced Pinyin | Reproducible; lexical exceptions become gaps. |
...
- authoritative stroke-type names aligned one-for-one with every Make Me a Hanzi
  SVG path;
- explanatory prose for every context-dependent one-to-many simplification
  beyond the attested mappings themselves;
...
- **Taiwan MOE public dictionary datasets — rejected for extraction.** They are
  CC BY-ND 3.0 Taiwan. Verbatim redistribution is allowed, but restructuring
  entries into this schema is an adaptation this audit does not approve.
...
- The remaining 97 `source_access_failed` entries stay in the asset manifest as
  ...
  represent delivered coverage gaps: all 214 radical records now resolve to a
...
After record-level mapping, Phase 2 integrates 4,243 historical originals.
Final radical coverage is 169/214 Oracle, 180/214 bronze, 204/214 六書通, and
214/214 seal. Every remaining empty historical array is a
...
null rather than guessed. The Phase 4 validator passes all 11 checks, including
...
Taiwan radical counts that differ from Unihan, 459 positional-radical arithmetic
