#!/usr/bin/env python3
"""Acquire every TE00010 glyph image for exact Traditional-primary radicals."""
...
SOURCE_ROOT = ROOT / "source-data" / "codh-liushutong-2026-08-10"
INDEX = SOURCE_ROOT / "radical-candidates.json"
...
def main() -> None:
    ...
    for position, radical in enumerate(index["radical_candidates"], start=1):
        number = radical["kangxi_number"]
        page_path = PAGES / f"{number:03d}.html"
        if page_path.exists():
            raw = page_path.read_bytes()
        else:
            raw, content_type = fetch(radical["source_character_page"], "text/html")
            if content_type != "text/html":
                raise RuntimeError(
                    f"unexpected page content type for radical {number}: {content_type}"
                )
        ...
        page_snapshots.append(
            {
                ...
                "bytes": len(raw),
                "url": radical["source_character_page"],
            }
        )
        text = raw.decode("utf-8")
        section_match = SECTION_RE.search(text)
        if section_match is None:
            raise RuntimeError(f"TE00010 section absent for radical {number}")
        section_candidates = []
        for variant_index, match in enumerate(
            IMAGE_RE.finditer(section_match.group(1)), start=1
        ):
            image_path = html.unescape(match.group(3))
            section_candidates.append(
                {
                    ...
                    "original_url": urllib.parse.urljoin(
                        radical["source_character_page"], image_path
                    ),
                }
            )
        if len(section_candidates) != radical["source_glyph_count"]:
            raise RuntimeError(
                f"radical {number}: parsed {len(section_candidates)} TE00010 images, "
                f"index declares {radical['source_glyph_count']}"
            )
        image_candidates.extend(section_candidates)
        print(
            f"TE00010 character pages {position}/{index['radical_candidate_count']}",
            flush=True,
        )
    ...
    def write_log() -> None:
        entries = sorted(
            entry_by_url.values(),
            key=lambda item: (item["kangxi_number"], item["variant_index"]),
        )
        failures = sorted(
        ...
        payload = {
            ...
            "request_policy": (
                ...
                "requests; source-published JPEG bytes are preserved unchanged."
            ),
            "selection_policy": (
                "Every TE00010 glyph image for every exact Traditional-primary "
                "Kangxi radical codepoint in the pinned book index."
            ),
    ...
    for index_number, candidate in enumerate(image_candidates, start=1):
        ...
        asset_id = f"codh-liushutong-{codepoint}-{variant_index:03d}"
        target = (
            ROOT
            / "assets"
            / "liushutong"
        ...
        entry_by_url[original_url] = {
            ...
            "edition": "偏類六書通 TE00010",
            "variant_index": variant_index,
            "mapping_method": "exact_traditional_primary_unicode_index_and_TE00010_anchor",
            "transformations": [],
            "representation_note": (
        ...
        write_log()
        if index_number % 20 == 0 or index_number == len(image_candidates):
            print(
                f"TE00010 originals {index_number}/{len(image_candidates)}",
                flush=True,
            )
    write_log()
    print(
        f"TE00010 acquisition complete: {len(entry_by_url)}/{len(image_candidates)} "
