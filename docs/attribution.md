...
through Phase 6. Asset-level file pages, creators, license URLs, revision dates,
...
## Taiwan MOE Concised Mandarin Dictionary readings and definitions

Canonical word Pinyin and Zhuyin for exact covered Traditional headwords are
copied verbatim from the Ministry of Education, Republic of China (Taiwan),
*Concised Mandarin Dictionary*, version `2014_20260626`, available at
[dict.concised.moe.edu.tw](https://dict.concised.moe.edu.tw/), under the
[Creative Commons Attribution-NoDerivatives 3.0 Taiwan
license](https://creativecommons.org/licenses/by-nd/3.0/tw/).

Required attribution: 中華民國教育部（Ministry of Education, R.O.C.）。《國語辭典簡編本》
（版本編號：2014_20260626）。

The integration uses exact NFC Traditional-headword matches and preserves each
published Pinyin, Zhuyin, and complete definition-cell string with its MOE entry
ID. The only definition transform is standards-defined decoding of OOXML
`_xHHHH_` character escapes; whitespace, ordering, wording, compatibility
ideographs, and sense structure are not edited. Differing CC-CEDICT readings and
mechanically derived CNS Zhuyin remain separately labeled comparison evidence.
A missing Concised Dictionary headword is treated as unresolved coverage, not as
proof that the existing reading or word is invalid.

## Taiwan MOE Revised Mandarin Dictionary readings and definitions

Exact Traditional headwords absent from the Concised Dictionary are checked
against the Ministry of Education, Republic of China (Taiwan), *Revised Mandarin
Dictionary*, version `2015_20260625`, available at
[dict.revised.moe.edu.tw](https://dict.revised.moe.edu.tw/), under the [Creative
Commons Attribution-NoDerivatives 3.0 Taiwan
license](https://creativecommons.org/licenses/by-nd/3.0/tw/).

Required attribution: 中華民國教育部（Ministry of Education, R.O.C.）。《重編國語辭典修訂本》
（版本編號：2015_20260625）。

The corpus copies exact Pinyin, Zhuyin, complete decoded definition-cell text,
and entry IDs without editorial alteration. It uses Revised Dictionary word
data only after an exact Concised Dictionary miss. Character readings use the
Revised Dictionary for 1,964 records; 36 lexical/character inventory differences
are adjudicated by exact-codepoint 正字 entries from the MOE Dictionary of
Variants, with all displaced readings preserved as conflicts.

## Taiwan canonical readings and comparison stroke counts

Taiwan-first Pinyin, Zhuyin, radical assignments, and stroke-sequence counts use the [CNS11643
...
## PRC 2013 standard-character table verification

All 37 conflicting character-level Simplified candidates were visually reviewed
against the PRC Ministry of Education's official [2013 General Standard Chinese
Character Table download page](https://www.moe.gov.cn/jyb_sjzl/ziliao/A19/201306/t20130601_186002.html).
The page, ZIP, and PDF are retained locally as verification evidence only. Under
the Ministry website's [copyright
statement](https://www.moe.gov.cn/jyb_sy/s3634/201005/t20100504_87292.html), the
original source files and derived page/glyph images are excluded from the
redistributable package. Delivered records contain only factual correspondence
decisions, source ID, conflict values, and the separately authored audit
metadata; `sources.json` records the exact official hashes and restrictions.
